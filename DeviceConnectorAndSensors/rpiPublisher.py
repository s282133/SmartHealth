...
if __name__ == "__main__":
    ...
    while True:
        if counter % POLLING_PERIOD_HR == 0:
            ...
            rpi.myPublish("P4IoT/SmartHealth/heartrate", json.dumps({"bn": "http://example.org/heartrateSensor/", "e": [{"n": "heartrate", "u": "bpm", "t": timeOfMessage, "v": newMeasureHR}]}))
            print(f"Published {newMeasureHR} with topic: P4IoT/SmartHealth/heartrate")
        if counter % POLLING_PERIOD_PRESSURE == 0:
            newMeasurePressure = int(rpi.getPressuremeasure(counter))
            timeOfMessage = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            rpi.myPublish("P4IoT/SmartHealth/pressure", json.dumps({"bn": "http://example.org/pressureSensor/", "e": [{"n": "pressure", "u": "hPa", "t": timeOfMessage, "v": newMeasurePressure}]}))
            print(f"Published {newMeasurePressure} with topic: P4IoT/SmartHealth/pressure")
        counter = counter + 1
